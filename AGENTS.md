...
- React画面のバンドル: `deno task ui:build`
...
- `run.sh` はmacOS標準の古いBashでも動作させる。空配列の展開など、`set -u`
  と衝突する書き方を避ける。
- `desktop/ui.bundle.js` はReact画面の生成物でGit管理しない。`desktop/ui.tsx`、 `desktop/ui/` または
  `desktop/ui_state.js` を変更し、`deno task ui:build` で生成する。

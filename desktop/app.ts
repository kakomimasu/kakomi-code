...
async function improveWithAgent(value: unknown) {
  ...
  const openCodeWorkDir = request.agent === "opencode"
    ? await createOpenCodeWorkspace(versionDir)
    : undefined;
  const agentCommand = openCodeWorkDir
    ? codingAgentCommand(request.agent, openCodeWorkDir, prompt, request.model)
    : initialAgentCommand;
  ...
  try {
    ...
    if (openCodeWorkDir) {
      const stagedMain = await validateOpenCodeWorkspace(openCodeWorkDir);
      const denoCommand = await findExecutable("deno");
      ...
      await Deno.copyFile(stagedMain, join(versionDir, "main.ts"));
      addCodingAgentStatus("main.tsの型チェックに成功し、変更を反映しました。");
    }
    ...
    codingAgentStopRequested = false;
    if (openCodeWorkDir) {
      await Deno.remove(openCodeWorkDir, { recursive: true }).catch(() => {});
    }
  }
}
